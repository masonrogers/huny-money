...
18. **Adding a new schema column — also append it to `applyAdditiveMigrations`.** **drizzle-kit 0.28 vs Postgres 18 incompatibility.** DO's managed Postgres cluster `db-postgresql-nyc3-00644` runs **pg18**, which gives every NOT NULL column a named constraint row in `pg_constraint` (e.g., `orders_id_not_null`). drizzle-kit 0.28 doesn't understand these — it sees them as drift, generates `ALTER TABLE ... DROP CONSTRAINT <name>` statements, and Postgres refuses to drop NOT NULL on PK columns with `42P16: column "id" is in a primary key`. The error is ignored (npm start runs anyway), but it means push silently fails to apply ANY changes — including pure column adds. `scripts/migrate-v1-to-v3.ts` runs an `applyAdditiveMigrations()` step BEFORE drizzle-kit that issues `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` for every additive column. Add new columns there too — schema.ts alone is not enough. Type changes / index renames / FK changes still go through drizzle-kit and may silently fail. The proper long-term fix is either upgrading drizzle-kit (verify pg18 named-NOT-NULL support before bumping) OR replacing `drizzle-kit push` in the deploy run_command with hand-rolled migrations — neither has been done yet. Local dev (pg16 in docker / CI Postgres service) is unaffected.

There is also pre-existing data drift in the prod `positions` table: `asset`, `direction`, `entry_price`, `entry_time` are nullable in prod but `notNull()` in schema.ts. drizzle-kit's diff plan would emit `SET NOT NULL` for these, which would itself work — but it gets killed by the 42P16 first. Code paths assume non-null; if you ever encounter a null position row, that's why.
